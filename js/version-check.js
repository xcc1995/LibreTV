...
async function checkForUpdates() {
  try {
    ...
    const VERSION_URL = {
      PROXY:
        "https://raw.ihtw.moe/raw.githubusercontent.com/LibreSpark/LibreTV/main/VERSION.txt",
      DIRECT:
...
function createErrorVersionElement(errorMessage) {
  ...
  errorElement.className =
    "text-gray-500 text-sm mt-1 text-center md:text-left";
...
function addVersionInfoToFooter() {
  checkForUpdates()
    ...
    .catch((error) => {
      console.error("版本检测出错:", error);
      // 创建错误版本信息元素并显示
      const errorElement = createErrorVersionElement(
        `错误信息: ${error.message}`
      );
...
function displayVersionElement(element) {
  ...
  const footerElement = document.querySelector(
    ".footer p.text-gray-500.text-sm"
  );
  if (footerElement) {
    // 在原版权信息后插入版本信息
